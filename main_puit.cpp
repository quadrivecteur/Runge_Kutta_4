...
{
    cout<<"Choisissez la valeur maximale du potentiel (eV) : ";
    cin>>V0;
    do
    {
        cout<<endl<<"Choisissez la largeur du potentiel entre 0 et 6 : ";
        cin>>largeur;
    }
    while (largeur<0 or largeur>6);
}
		  ...
		  long double e, double h_barre, double eV, double largeur)
{
        ...
        if (x>=-largeur/2 and x<=largeur/2)
        {
            v=0;
            vo=eV*sqrt(2*masse*abs(v-e))/h_barre;
        }
...
// Calcul de la fonction d'onde pour une energie donnee
...
void ecriture_tableau (long double explosion_min[][2], double points)
{
  fstream f;
  // ON ECRIT L'EXPLOSION EN FONCTION DE L'ENERGIE
  f.open ("puits_explosion.txt", ios::out);
  for (int i = 0; i < points; i++)
    {
...
void balayage (double V0, double largeur, int points)
{
  double h = 0.008;		// Pas utilise pour la methode RK4
  ...
  for (int i = 0; i < 1000; i++)
    {
      ...
      // On ecrit la valeur de l'energie et l'explosion en x=4 dans un tableau
  ...
  ecriture_tableau (explosion_min, points);
  ...
  fstream f;
  f.open ("Valeurs_propres_puits.txt", ios::out);
  for (int i = 0; i < 2 * j; i++)
    {
...
int main ()
{
  int points = 1000;    // Discretisation de l'axe X
  double V0;			// Valeur maximale du potentiel (en eV)
  double largeur;		// Largeur du potentiel

  choix_initial(V0,largeur); // caracteristiques du potentiel
  
  balayage (V0, largeur, points); // balayage en energie
